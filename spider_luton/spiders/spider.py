...
class SpiderLuton(scrapy.Spider):
    ...
    def _initialize_item(self, response, xpath_strings):
        ...
        listing_type = response.xpath(
            self._get_xpath_str(xpath_strings, 'listing_type')).extract_first()
        if listing_type:
            item['listing_type'] = listing_type.strip()[4:].lower()
        item['agent'] = []
        agent_number = response.xpath(self._get_xpath_str(xpath_strings, 'agent_count')).extract_first()
        agent_count = 1
        while True:
            agent_name = response.xpath(
                self._get_xpath_str(xpath_strings, 'agent_name').format(agent_count)).extract_first()
            agent_email = response.xpath(
                self._get_xpath_str(xpath_strings, 'agent_email').format(agent_count)).extract_first()
